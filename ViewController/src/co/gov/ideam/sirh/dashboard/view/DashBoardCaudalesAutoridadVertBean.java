...
public class DashBoardCaudalesAutoridadVertBean  extends StandarDashBoard{
    ...
    public DashBoardCaudalesAutoridadVertBean(){
        FacesUtils.removeManagedBeanFromSession("dashBoard");
        
        FacesUtils.setActiveBean("dashBoardCaudalesAutoridadVert", "DashBoardCaudalesAutoridadVertBean",
                                 DashBoardCaudalesAutoridadVertBean.class);
       this.load();        
    }
         ...
         try{
             UsuarioConectadoTO usuarioConectado = 
                 ...
                 if(usuarioConectado==null){
                     listaCaudales= uad.getListaCaudalesVertidos(null);
                    ...
                    listaCaudales= uad.getListaCaudalesVertidos(usuarioConectado.getUsuario().
    ...
    public void detalle_actionListener(ActionEvent actionEvent) {
        try{    
            listaCaudalesDetalle= new ArrayList();
            t_caudales_detalle.getChildren().clear();
            Long codigo = (Long)actionEvent.getComponent().getAttributes().get("codigo");   
            Long codigoTramo = (Long)actionEvent.getComponent().getAttributes().get("codigoTramo");   
            
            if(t_caudales_detalle != null){                                    
                for(int i=0; i<t_caudales_detalle.getChildCount(); i++){                
                    t_caudales_detalle.getChildren().remove(i);                
                }
            }   
            System.out.println("codigo fuente caudales conc:"+codigo);
            System.out.println("codigoTramo caudales conc:"+codigoTramo);
            UsuarioConectadoTO usuarioConectado = 
                (UsuarioConectadoTO)FacesUtils.getFromSession("usuarioConectado");
                ...
                if(usuarioConectado==null){
                    ...
                    this.listaCaudalesDetalle= uad.getListaCaudalesVertidosDetalle(null, codigo, codigoTramo);
                    } else {   
                   ...
                   this.listaCaudalesDetalle= uad.getListaCaudalesVertidosDetalle(usuarioConectado.getUsuario().
                                                   getAutoridadAmbiental().
                                                   getId().
                                                   longValue(),codigo, codigoTramo);
                } 
          }

            t_caudales_detalle.setRendered(true);
            AdfFacesContext.getCurrentInstance().addPartialTarget(t_caudales_detalle);
        }catch(IdeamException e){
            showMessage(e);
        }
        AdfFacesContext.getCurrentInstance().addPartialTarget(panelBoxCalidadGraf1); 
    }
