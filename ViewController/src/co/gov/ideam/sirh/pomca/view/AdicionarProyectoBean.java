...
public class AdicionarProyectoBean {
    ...
    private RichInputText it_nombrePrograma;
    ...
    public void setIt_nombrePrograma(RichInputText inputText1) {
        this.it_nombrePrograma = inputText1;
    }

    public RichInputText getIt_nombrePrograma() {
        return it_nombrePrograma;
    }
    ...
    public AdicionarProyectoBean() {

        FacesUtils.setActiveBean("adicionarProyectoBean", "Adicionar Proyecto",
                                 PomcaDelegate.class);
        load();
    ...
    private void load() {
        try {
            ...
            proyecto.setIdPrograma(programa.getId());
            proyecto.setCodigoAutoridad(autoridadAmbiental.getId().longValue());
            proyecto.setNombre("");
            proyecto.setDescripcion("");
            //proyecto.setFechaInicio(new Timestamp());
            //proyecto.setFechaVigencia(new Timestamp());
            proyecto.setPorcCumplimiento(0D);
            proyecto.setPorcEjecucion(0D);
            proyecto.setPresupuestoAsignado(new BigDecimal("0"));
    ...
    public void cbNext1_action() {
        ...
        if( it_nombre.getValue().toString().length()==0){
            showMessage("obligatorio",FacesMessage.SEVERITY_ERROR,this.it_nombre);       
            return;
        }        
        ...
        if (fi.compareTo(fi_p)<0 ||  fi.compareTo(ff_p) > 0 ) {
            showMessage("RANGO_FECHA_ERRADO_PROG",FacesMessage.SEVERITY_ERROR,this.id_fechaInicio);       
            return;
        }

        if (fv.compareTo(fi_p)< 0 ||  fv.compareTo(ff_p) > 0 ) {
            showMessage("RANGO_FECHA_ERRADO_PROG",FacesMessage.SEVERITY_ERROR,this.id_fechaVigencia);       
            return;
        }
    ...
    public void guardarProyecto(){
        try{
            ...
            proyecto.setCodigoAutoridad(usuarioConectado.getUsuario().getAutoridadAmbiental().getId().longValue());
            proyecto.setIdPrograma(programa.getId());
            proyecto.setNombre(it_nombre.getValue().toString().toUpperCase());
            PomcaDelegate pd = PomcaDelegate.getInstance();
          
            pd.addProyecto(proyecto);
