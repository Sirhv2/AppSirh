...
public class TipoConflictoBean extends StandarBean {

    private FnttFuente fuente;
    private String accionAdicionar;
    private FnttConflicto conflicto;
    
    private List listaTipoConflicto;
    private List listaTipoConflictoAsociado;

    private List listaOrigenConflicto;
    private List listaOrigenConflictoAsociado;
    ...
    private RichSelectManyListbox smlbTipoConflicto;
    private UISelectItems siTipoConflicto;
  ...
  public TipoConflictoBean() {
        FacesUtils.setActiveBean("tipoConflictoBean", "Tipo Conflicto",
                                 TipoConflictoBean.class);
    ...
    public void load() {
        try {
            ...
            conflicto.setListaTipoConflicto(fd.getTipoConflictoXConflicto(conflicto.getId()));
            conflicto.setListaOrigenConflicto(fd.getOrigenConflictoXConflicto(conflicto.getId()));
            
            ParametrosDelegate pd = ParametrosDelegate.getInstance();

            listaTipoConflicto = this.cargarParametro(ConstantesParametros.CATEGORIA_TIPO_CONFLICTO);
            listaOrigenConflicto = this.cargarParametro(ConstantesParametros.CATEGORIA_ORIGEN_CONFLICTO);

            this.listaTipoConflictoAsociado = new ArrayList();

            if(this.conflicto.getListaTipoConflicto()!=null){
                Iterator it = this.conflicto.getListaTipoConflicto().iterator();
                while(it.hasNext()){
                    TipoConflicto tipo = (TipoConflicto)it.next();
                    Lista lista = pd.getLista(tipo.getIdTipoConflcito());
                    this.listaTipoConflictoAsociado.add(lista);
                }
            }

            this.listaOrigenConflictoAsociado = new ArrayList();

            if(this.conflicto.getListaOrigenConflicto()!=null){
                Iterator it = this.conflicto.getListaOrigenConflicto().iterator();
                while(it.hasNext()){
                    OrigenConflicto origen = (OrigenConflicto)it.next();
                    Lista lista = pd.getLista(origen.getIdOrigenConflcito());
                    this.listaOrigenConflictoAsociado.add(lista);
                }
            }
    ...
    public void guardarConflicto() {
        try {
            ...
            if(this.conflicto.getListaTipoConflicto()!=null){
                for(TipoConflicto tipoConflicto : this.conflicto.getListaTipoConflicto()){
                    fd.deleteTipoConflicto(tipoConflicto);
                }
                this.conflicto.getListaTipoConflicto().clear();
            }

            if(this.conflicto.getListaOrigenConflicto()!=null){
                for(OrigenConflicto origenConflicto : this.conflicto.getListaOrigenConflicto()){
                    fd.deleteOrigenConflicto(origenConflicto);
                }
                this.conflicto.getListaOrigenConflicto().clear();
            }

            if(this.smlbTipoConflicto.getValue()!=null){
                Iterator it= ((List)this.smlbTipoConflicto.getValue()).iterator();
                while(it.hasNext()){
                    Lista lis = (Lista)it.next();
                    
                    TipoConflicto tipoConflicto = new TipoConflicto();
                    tipoConflicto.setIdConflicto(conflicto.getId());
                    tipoConflicto.setCodigoAutoridad( new Long(usuarioConectado.getUsuario().getAutoridadAmbiental().getId()) );
                    tipoConflicto.setIdTipoConflcito(lis.getCodigo());
                    
                    fd.createTipoConflicto(tipoConflicto);
                }
            }

            if(this.smlbOrigenConflicto.getValue()!=null){
                Iterator it= ((List)this.smlbOrigenConflicto.getValue()).iterator();
                while(it.hasNext()){
                    Lista lis = (Lista)it.next();
                    
                    OrigenConflicto origenConflicto = new OrigenConflicto();
                    origenConflicto.setIdConflicto(conflicto.getId());
                    origenConflicto.setCodigoAutoridad( new Long(usuarioConectado.getUsuario().getAutoridadAmbiental().getId()) );
                    origenConflicto.setIdOrigenConflcito(lis.getCodigo());
                    
                    fd.createOrigenConflicto(origenConflicto);
                }
            }
    ...
    public void setSmlbTipoConflicto(RichSelectManyListbox smlbPretratamiento) {
        this.smlbTipoConflicto = smlbPretratamiento;
    }

    public RichSelectManyListbox getSmlbTipoConflicto() {
        return smlbTipoConflicto;
    }

    public void setSiTipoConflicto(UISelectItems siPretratamiento) {
        this.siTipoConflicto = siPretratamiento;
    }

    public UISelectItems getSiTipoConflicto() {
        return siTipoConflicto;
    }

    public void setListaTipoConflicto(List listaTipoConflicto) {
        this.listaTipoConflicto = listaTipoConflicto;
    }

    public List getListaTipoConflicto() {
        return listaTipoConflicto;
    }

    public void setListaTipoConflictoAsociado(List listaTipoConflictoAsociado) {
        this.listaTipoConflictoAsociado = listaTipoConflictoAsociado;
    }

    public List getListaTipoConflictoAsociado() {
        return listaTipoConflictoAsociado;
    }
    ...
    public void setListaOrigenConflicto(List listaOrigenConflicto) {
        this.listaOrigenConflicto = listaOrigenConflicto;
    }

    public List getListaOrigenConflicto() {
        return listaOrigenConflicto;
    }

    public void setListaOrigenConflictoAsociado(List listaOrigenConflictoAsociado) {
        this.listaOrigenConflictoAsociado = listaOrigenConflictoAsociado;
    }

    public List getListaOrigenConflictoAsociado() {
        return listaOrigenConflictoAsociado;
    }
