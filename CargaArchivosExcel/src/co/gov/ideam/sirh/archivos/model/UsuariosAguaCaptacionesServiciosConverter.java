...
public class UsuariosAguaCaptacionesServiciosConverter extends ModelConverter{
    public UsuariosAguaCaptacionesServiciosConverter() {
        super();
    }
    ...
    private Concesion getConcesion(Concesion concesion)throws IdeamException{
        ...
        if(tmp.indexOf(",")!=-1)
        tmp=tmp.replaceAll(",",".");
        concesion.setCaudalConcesionado( Double.parseDouble( tmp) );
    ...
    private Predio getPredio(Predio predio)throws IdeamException{
        ...
        predio.setNombre( row.getCellValue("Nombre sitio").toString() );
        ...
        System.out.println("Municipio predio: "+nombreMunTxt);
        Divipola mun = 
            parametrosService.getDivipolaByName(nombreMunTxt, depto);
        ...
        String segundosLat =this.row.getCellValue("Segundos Latitud").toString();
            if(segundosLat.indexOf(",")!=-1)
            segundosLat=segundosLat.replaceAll(",",".");
        Double segundosLatitud = Double.parseDouble(segundosLat
                                     );
        ...
        String segundos =this.row.getCellValue("Segundos Longitud").toString();
            if(segundos.indexOf(",")!=-1)
            segundos=segundos.replaceAll(",",".");
        Double segundosLongitud = Double.parseDouble( segundos);
    ...
    private Captacion getCaptacion(Captacion captacion)throws IdeamException{
        ...
        String nombreDeptoTxt = this.row.getCellValue("Departamento Captacion").toString();
        Divipola depto =
            parametrosService.getDivipolaByName(nombreDeptoTxt,"DEP");
        captacion.setIdDepartamento(depto.getId().intValue());

        String nombreMunTxt = this.row.getCellValue("Municipio Captacion").toString();
        Divipola mun =
            parametrosService.getDivipolaByName(nombreMunTxt, depto);
        ...
        captacion.setNombreCentroPoblado(this.row.getCellValue("Nombre Centro Poblado Captacion").toString());
        ...
        System.out.println("sistemaReferencia de la captacion:"+sistemaReferencia.getCodigo());
        try{
                Double gradosLatitud = Double.parseDouble(
                                         this.row.getCellValue("Grados Latitud Captacion").toString());
                captacion.setGradosLat(gradosLatitud.intValue());
                System.out.println("gradosLatitud de la captacion:"+gradosLatitud);
        }catch(Exception c){

                      System.out.println("gradosLatitud error:"+c);
        ...
        String segundoslat =this.row.getCellValue("Segundos Latitud Captacion").toString();
            if(segundoslat.indexOf(",")!=-1)
            segundoslat=segundoslat.replaceAll(",",".");
        Double segundosLatitud = Double.parseDouble(segundoslat
                                     );
        ...
        String segundoslon =this.row.getCellValue("Segundos Longitud Captacion").toString();
            if(segundoslon.indexOf(",")!=-1)
            segundoslon=segundoslon.replaceAll(",",".");
        Double segundosLongitud = Double.parseDouble(segundoslon
                                     );
