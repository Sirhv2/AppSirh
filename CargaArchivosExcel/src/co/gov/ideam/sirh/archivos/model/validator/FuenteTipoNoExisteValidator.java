...
public class FuenteTipoNoExisteValidator implements CompositeValidatorInterface, RequiresAutoridadValidatorInterface, Serializable {
    ...
    public FuenteTipoNoExisteValidator() {
        super();
    }
    
    public FuenteTipoNoExisteValidator(String columnaPadre, Long categoria) {
        super();
        ...
        if(this.campos !=null && columnaPadre!=null){
            ...
            if( nombrePadre != null && nombrePadre.length()>0){
                ...
                try{
                    // Validar el tipo
                    Lista tipo = 
                        this.getParametrosService().getListaByDescripcion(nombrePadre.trim().
                                                                toUpperCase(), 
                                                                codigoCategoria);
                    ...
                    FnttFuente existe =  
                        this.getFuentesService().getFuente(value.toString().toUpperCase().trim(), 
                                                 this.getAutoridad(), tipo.getCodigo());
                    if(existe==null){
                        error = "La fuente " + value + " no se encuentra registrada para la autoridad " + this.getAutoridad().getNombre();
                        return false;                                    
                    } 
                    return true;                
                }catch(IdeamException e){
    ...
    private FuentesEJB getFuentesService()throws IdeamException{
        try {
            ...
            throw new IdeamException("FuenteTipoNoExisteValidator Imposible ubicar el EJB de Fuentes " + 
                                     e.getMessage());
        }  
